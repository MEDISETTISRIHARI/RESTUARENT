...
async function initPostgres() {
  ...
  try {
    ...
    dbMode = 'postgres';
    console.log('✓ PostgreSQL connection established (Supabase)');
    return true;
  } catch (err) {
...
CREATE INDEX IF NOT EXISTS idx_orders_tracking_id ON orders(tracking_id) WHERE is_deleted != 1;
`;
...
class PostgresDB {
  ...
  async run(sql, params = []) {
    ...
    const result = await this.pool.query(this._format(sql), params);
    if (result.rows.length > 0 && result.rows[0].id !== undefined) {
      this._lastInsertRowid = result.rows[0].id;
    }
    return { changes: result.rowCount, lastInsertRowid: this._lastInsertRowid };
  }
...
async function getColumns(tableName) {
  ...
  return db.exec('PRAGMA table_info(' + tableName + ')')[0]?.values.map(v => v[1]) || [];
}
...
async function migrate() {
  ...
  const settingsCols = await getColumns('restaurant_settings');
  if (!settingsCols.includes('payment_gateway')) {
    await db.run("ALTER TABLE restaurant_settings ADD COLUMN payment_gateway TEXT DEFAULT 'razorpay'");
  }
  if (!settingsCols.includes('payment_gateway_key')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_gateway_key TEXT');
  }
  if (!settingsCols.includes('payment_gateway_secret')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_gateway_secret TEXT');
  }
  if (!settingsCols.includes('upi_id')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN upi_id TEXT');
  }
  if (!settingsCols.includes('payment_upi_enabled')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_upi_enabled INTEGER DEFAULT 1');
  }
  if (!settingsCols.includes('payment_upi_name')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_upi_name TEXT');
  }
  if (!settingsCols.includes('payment_qr_enabled')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_qr_enabled INTEGER DEFAULT 0');
  }
  if (!settingsCols.includes('payment_qr_image')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_qr_image TEXT');
  }
  if (!settingsCols.includes('payment_bank_enabled')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_bank_enabled INTEGER DEFAULT 0');
  }
  if (!settingsCols.includes('payment_bank_holder')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_bank_holder TEXT');
  }
  if (!settingsCols.includes('payment_bank_name')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_bank_name TEXT');
  }
  if (!settingsCols.includes('payment_bank_account')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_bank_account TEXT');
  }
  if (!settingsCols.includes('payment_bank_ifsc')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_bank_ifsc TEXT');
  }
  if (!settingsCols.includes('payment_bank_branch')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_bank_branch TEXT');
  }
  if (!settingsCols.includes('payment_direct_enabled')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_direct_enabled INTEGER DEFAULT 0');
  }
  if (!settingsCols.includes('payment_direct_number')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_direct_number TEXT');
  }
  if (!settingsCols.includes('payment_direct_name')) {
    await db.run('ALTER TABLE restaurant_settings ADD COLUMN payment_direct_name TEXT');
  }

  const orderItemsCols = await getColumns('order_items');
  if (!orderItemsCols.includes('promotion_discount')) {
    await db.run('ALTER TABLE order_items ADD COLUMN promotion_discount REAL DEFAULT 0');
  }

  const paymentsCols = await getColumns('payments');
  if (!paymentsCols.includes('payment_reference')) {
    await db.run('ALTER TABLE payments ADD COLUMN payment_reference TEXT');
  }
  if (!paymentsCols.includes('payment_proof')) {
    await db.run('ALTER TABLE payments ADD COLUMN payment_proof TEXT');
  }
  if (!paymentsCols.includes('rejection_reason')) {
    await db.run('ALTER TABLE payments ADD COLUMN rejection_reason TEXT');
  }
  if (!paymentsCols.includes('verified_by')) {
    await db.run('ALTER TABLE payments ADD COLUMN verified_by INTEGER');
  }
  if (!paymentsCols.includes('verified_at')) {
    await db.run('ALTER TABLE payments ADD COLUMN verified_at DATETIME');
  }
  if (!paymentsCols.includes('updated_at')) {
    await db.run('ALTER TABLE payments ADD COLUMN updated_at DATETIME');
  }

  const customerCols = await getColumns('customers');
  if (!customerCols.includes('password_hash')) {
    await db.run('ALTER TABLE customers ADD COLUMN password_hash TEXT');
  }
  if (!customerCols.includes('is_deleted')) {
    await db.run('ALTER TABLE customers ADD COLUMN is_deleted INTEGER DEFAULT 0');
  }
  if (!customerCols.includes('deleted_at')) {
    await db.run('ALTER TABLE customers ADD COLUMN deleted_at DATETIME');
  }
  if (!customerCols.includes('deleted_by')) {
    await db.run('ALTER TABLE customers ADD COLUMN deleted_by INTEGER');
  }
  ...
  if (!orderCols.includes('is_deleted')) {
    await db.run('ALTER TABLE orders ADD COLUMN is_deleted INTEGER DEFAULT 0');
  }
  if (!orderCols.includes('deleted_at')) {
    await db.run('ALTER TABLE orders ADD COLUMN deleted_at DATETIME');
  }
  if (!orderCols.includes('deleted_by')) {
    await db.run('ALTER TABLE orders ADD COLUMN deleted_by INTEGER');
  }
  await db.run('CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_tracking_id ON orders(tracking_id) WHERE is_deleted != 1');

  await db.run("UPDATE orders SET tracking_id = order_number WHERE tracking_id IS NULL");
  await db.run('UPDATE orders SET is_deleted = 0 WHERE is_deleted IS NULL');

  const promoCols = await getColumns('promotions');
  if (!promoCols.includes('id')) {
    await db.run(`CREATE TABLE promotions (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      name TEXT NOT NULL,
      description TEXT,
      discount_type TEXT NOT NULL CHECK (discount_type IN ('percentage', 'fixed')),
...
async function seedData() {
  if (isPostgres()) {
    const adminCount = (await db.all('SELECT COUNT(*) as count FROM admin_users'))[0]?.count || 0;
    if (adminCount === 0) {
      const hash = bcrypt.hashSync('admin123', 10);
      await db.run('INSERT INTO admin_users (username, email, password_hash) VALUES (?, ?, ?)', ['admin', 'admin@restaurant.com', hash]);
      console.log('✓ Default admin created: admin / admin123');
    }

    const settingsCount = (await db.all('SELECT COUNT(*) as count FROM restaurant_settings'))[0]?.count || 0;
    if (settingsCount === 0) {
      await db.run(`INSERT INTO restaurant_settings (id, name, tagline, description, address, phone, email, opening_time, closing_time, is_open, manual_status, delivery_charge, min_order_value, tax_rate, payment_gateway) VALUES (1, 'Spice Garden', 'Authentic Indian Flavors', 'Welcome to Spice Garden - serving delicious authentic Indian cuisine with love since 2010.', '123 Main Street, Indiranagar, Bengaluru 560038', '+91 98765 43210', 'hello@spicegarden.com', '09:00', '22:00', 1, 1, 40, 100, 5, 'razorpay')`);
      console.log('✓ Default restaurant settings created');
    }

    const homepageCount = (await db.all('SELECT COUNT(*) as count FROM homepage_content'))[0]?.count || 0;
    if (homepageCount === 0) {
      await db.run(`INSERT INTO homepage_content (id, hero_title, hero_subtitle, hero_description, hero_button_text, hero_button_link, about_title, about_description, offer_title, offer_description, footer_about, footer_copyright) VALUES (1, 'Delicious Food Delivered Hot & Fresh', 'Order from the best restaurant in town', 'Experience authentic flavors crafted with love. From sizzling starters to decadent desserts, we bring the finest dining experience to your doorstep.', 'Order Now', '/menu', 'Our Story', 'Spice Garden has been serving the community for over a decade. Our chefs use only the freshest ingredients and traditional recipes to create unforgettable meals.', 'Special Offers', 'Get 20% off on your first order! Use code WELCOME20 at checkout.', 'Spice Garden is your go-to destination for delicious, authentic Indian cuisine delivered hot and fresh to your doorstep.', '© 2026 Spice Garden. All rights reserved.')`);
      console.log('✓ Default homepage content created');
    }

    const catCount = (await db.all('SELECT COUNT(*) as count FROM categories'))[0]?.count || 0;
    if (catCount === 0) {
      const cats = [
      ...
      for (const c of cats) {
        await db.run('INSERT INTO categories (name, slug, description, sort_order) VALUES (?, ?, ?, ?)', c);
      }
      console.log('✓ Default categories created');
    }

    const foodCount = (await db.all('SELECT COUNT(*) as count FROM food_items'))[0]?.count || 0;
    if (foodCount === 0) {
      const foods = [
      ...
      [5, 'Chicken Hakka Noodles', 'chicken-hakka-noodles', 'Stir-fried noodles with chicken and vegetables', 230, 0, 0, 1, 1, 1, 'Noodles, chicken, vegetables, soy sauce, eggs', 15],
      ...
      for (const f of foods) {
        await db.run(`INSERT INTO food_items (category_id, name, slug, description, price, discount_price, is_veg, is_available, is_featured, is_popular, ingredients, preparation_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, f);
      }
      console.log('✓ Default food items created');
    }

    const couponCount = (await db.all('SELECT COUNT(*) as count FROM coupons'))[0]?.count || 0;
    if (couponCount === 0) {
      await db.run(`INSERT INTO coupons (code, description, discount_type, discount_value, min_order_value, max_discount, expiry_date, usage_limit, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, ['WELCOME20', '20% off on your first order', 'percentage', 20, 200, 100, '2027-12-31', 1000, 1]);
      await db.run(`INSERT INTO coupons (code, description, discount_type, discount_value, min_order_value, max_discount, expiry_date, usage_limit, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, ['FLAT50', 'Flat ₹50 off on orders above ₹500', 'fixed', 50, 500, null, '2027-12-31', 500, 1]);
      console.log('✓ Default coupons created');
    }

    const deliveryCount = (await db.all('SELECT COUNT(*) as count FROM delivery_persons'))[0]?.count || 0;
    if (deliveryCount === 0) {
      await db.run('INSERT INTO delivery_persons (name, phone, vehicle_type, is_active) VALUES (?, ?, ?, ?)', ['Rahul Kumar', '+91 90000 11111', 'Bike', 1]);
      await db.run('INSERT INTO delivery_persons (name, phone, vehicle_type, is_active) VALUES (?, ?, ?, ?)', ['Amit Singh', '+91 90000 22222', 'Bike', 1]);
...
function wrapSqliteDb() {
  ...
  db.lastInsertRowid = async function () {
...
async function init() {
  ...
  console.log('✓ Database initialized (' + dbMode + ')');
  return db;
}
...
module.exports = {
  ...
  getDb: () => db,
  persist,
  wrapDb: () => { if (!isPostgres()) wrapSqliteDb(); },
  isPostgres,
  dbMode,
  SCHEMA_POSTGRES
};
